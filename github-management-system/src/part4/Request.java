...
//以下开始获取json数据并且进行解析
        try (CloseableHttpClient httpClient = HttpClientBuilder.create()
            ...
            File f2=new File("C:\\Program Files\\Git\\2021-Spring-Tour\\github-management-system\\src\\part4","repos.txt");
            try (FileWriter writefile = new FileWriter(f2)) {
                for (int j = 1; j < jsonArray.size()+1; j++) {
                    ...
                    writefile.write("star中第" + j + "个仓库的URL:    " + jsonObject.getString("html_url") + "\n");
                    writefile.write("star中第" + j + "个仓库的创建时间:   " + jsonObject.getString("created_at") + "\n");
                    writefile.write("star中第" + j + "个仓库的最近一次的更新时间:   " + jsonObject.getString("updated_at") + "\n");
                    writefile.write("star中第" + j + "个仓库的最近一次的提交时间:   " + jsonObject.getString("pushed_at") + "\n");
                    writefile.write("star中第" + j + "个仓库的简介:   " + jsonObject.getString("description") + "\n");
                    writefile.write("star中第" + j + "个仓库的所使用的语言:   " + jsonObject.getString("language") + "\n");
                    writefile.write("star中第" + j + "个仓库的star量:   " + jsonObject.getString("stargazers_count") + "\n");
                    writefile.write("star中第" + j + "个仓库的字节数:   " + jsonObject.getString("size") + "\n");
                }
